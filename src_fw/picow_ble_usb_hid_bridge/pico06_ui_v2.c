...
static uint16_t screen_background(void)
{
    switch (g_screen) {
        ...
        case SCREEN_SUCCESS: return COLOR_DARK_CYAN;
                                           ...
                                           bool *found)
{
    if (config != NULL) {
        for (uint8_t i = 0; i < REMAP_SOURCE_COUNT; ++i) {
            if (config->mappings[i] == (uint8_t)target) {
                *found = true;
...
static bool pending_source_is_applied(remap_source_t source)
{
    ...
    if (source == REMAP_SOURCE_BACK &&
        g_pending.drag_fix_back != g_profile.active.drag_fix_back) return false;
    if (source == REMAP_SOURCE_FORWARD &&
        g_pending.drag_fix_forward != g_profile.active.drag_fix_forward) return false;
                                 ...
                                 remap_target_t target)
{
    ...
    const uint16_t color = found && pending_source_is_applied(source)
                             ? COLOR_CYAN : COLOR_WHITE;
    set_line(line_index, color, "%s: %s", target_name,
             found ? source_short(source) : "PASS");
}
...
static int applied_drag_selection(void)
{
    ...
    const device_drag_fix_policy_t policy = g_drag_source == REMAP_SOURCE_BACK
        ? g_profile.active.drag_fix_back : g_profile.active.drag_fix_forward;
...
static void build_help(void)
{
    ...
    if (g_help_return == SCREEN_DRAG) {
        set_line(1, COLOR_WHITE, "AUTO = KNOWN FIX");
        set_line(2, COLOR_WHITE, "FORCE = TRY HIDPP");
        set_line(3, COLOR_WHITE, "OFF = STANDARD HID");
        set_line(4, COLOR_WHITE, "OFF HAS NO HOLD FIX");
        set_line(5, COLOR_WHITE, "APPLY SAVES CHANGES");
        set_line(7, COLOR_GRAY, "K1 BACK");
        return;
    }
    ...
    if (g_help_return == SCREEN_CUSTOM || g_help_return == SCREEN_CAPTURE ||
        ...
        set_line(1, COLOR_WHITE, "JOY LEFT -> LEFT");
        set_line(2, COLOR_WHITE, "JOY RIGHT -> RIGHT");
        set_line(3, COLOR_WHITE, "JOY UP -> FORWARD");
        set_line(4, COLOR_WHITE, "JOY DOWN -> BACK");
        set_line(5, COLOR_WHITE, "JOY PRESS -> MIDDLE");
        set_line(6, COLOR_WHITE, "K2 ACTIONS");
        set_line(7, COLOR_GRAY, "K1 BACK");
        return;
    }

    set_line(1, COLOR_WHITE, "JOY = LEFT OF LCD");
    set_line(2, COLOR_WHITE, "K1=A BACK/CANCEL");
    set_line(3, COLOR_WHITE, "K2=B MOUSE REMAP");
    set_line(4, COLOR_WHITE, "K3=X HELP");
    set_line(5, COLOR_WHITE, "K4=Y SCREEN LOCK");
    set_line(6, COLOR_WHITE, "JOY PRESS = STATUS");
    set_line(7, COLOR_GRAY, "K1 BACK");
}
...
{
    switch (g_apply_kind) {
        case APPLY_KIND_PASSTHROUGH:
            set_line(1, COLOR_WHITE, "PASSTHROUGH PROFILE");
            set_line(2, COLOR_WHITE, "APPLIED SUCCESSFULLY");
            set_line(3, COLOR_WHITE, "ORIGINAL MOUSE");
            set_line(4, COLOR_WHITE, "BUTTONS ARE ACTIVE");
            break;
        case APPLY_KIND_DEFAULT:
            set_line(1, COLOR_WHITE, "DEFAULT PROFILE");
            set_line(2, COLOR_WHITE, "APPLIED SUCCESSFULLY");
            set_line(3, COLOR_WHITE, "LEFT NOW ESCAPE");
            set_line(4, COLOR_WHITE, "FWD NOW LEFT CLICK");
            break;
        case APPLY_KIND_CUSTOM:
            set_line(1, COLOR_WHITE, "CUSTOM ACTIONS");
            set_line(2, COLOR_WHITE, "APPLIED SUCCESSFULLY");
            set_line(3, COLOR_WHITE, "CUSTOM MAP SAVED");
            set_line(4, COLOR_WHITE, "FOR THIS MOUSE");
            break;
        case APPLY_KIND_CLEAR:
            set_line(1, COLOR_WHITE, "CLEAR MAPPINGS");
            set_line(2, COLOR_WHITE, "APPLIED SUCCESSFULLY");
            set_line(3, COLOR_WHITE, "CUSTOM BUTTONS NOW");
            set_line(4, COLOR_WHITE, "PASSTHROUGH");
            break;
        default:
            set_line(2, COLOR_WHITE, "APPLIED SUCCESSFULLY");
            break;
    }
    set_line(6, COLOR_GRAY, "K1 BACK TO OPTIONS");
    set_line(7, COLOR_GRAY, "JOY PRESS STATUS");
}
...
static void build_lines(void)
{
    ...
    switch (g_screen) {
        case SCREEN_HOME:
            set_title("HOME");
            set_line(1, COLOR_WHITE, "JOY = LEFT OF LCD");
            set_line(2, COLOR_WHITE, "PRESS = JOY CENTER");
            set_line(3, COLOR_WHITE, "K1=A BACK/CANCEL");
            set_line(4, COLOR_WHITE, "K2=B MOUSE REMAP");
            set_line(5, COLOR_WHITE, "K3=X HELP");
            set_line(6, COLOR_WHITE, "K4=Y SCREEN LOCK");
            set_line(7, COLOR_GRAY, "JOY PRESS = STATUS");
            break;
        ...
        case SCREEN_STATUS: {
            ...
            if (!g_have_profile || !g_profile.connected) {
                set_line(1, COLOR_WHITE, "WAITING FOR MOUSE");
                set_line(2, COLOR_CYAN, "PROFILE PASSTHRU");
            } else {
                set_line(1, COLOR_CYAN, "PROFILE %s",
                         remap_profile_mode_name(g_profile.active.mode));
                char fwd[UI_TEXT_MAX + 1u];
                char back[UI_TEXT_MAX + 1u];
                build_drag_status(fwd, sizeof(fwd), true);
                build_drag_status(back, sizeof(back), false);
                set_line(2, COLOR_WHITE, "%s", fwd);
                set_line(3, COLOR_WHITE, "%s", back);
                set_line(4, COLOR_WHITE, "STORAGE %s",
                         g_profile.storage_ok ? "OK" : "ERROR");
            }
            set_line(5, COLOR_GRAY, "K2=B MOUSE REMAP");
            set_line(6, COLOR_GRAY, "K3=X HELP");
            set_line(7, COLOR_GRAY, "K1=A HOME");
            break;
        }

        case SCREEN_MODE_MENU: {
            set_title("MOUSE REMAP ACTION");
            const device_profile_mode_t active = g_have_profile
                ? g_profile.active.mode : DEVICE_PROFILE_PASSTHROUGH;
            set_line(1, option_color(g_mode_selection == 0u,
                                     active == DEVICE_PROFILE_PASSTHROUGH),
                     "%c APPLY PASSTHROUGH", g_mode_selection == 0u ? '>' : ' ');
            set_line(2, option_color(g_mode_selection == 1u,
                                     active == DEVICE_PROFILE_DEFAULT_REMAP),
                     "%c APPLY DEFAULT", g_mode_selection == 1u ? '>' : ' ');
            set_line(3, option_color(g_mode_selection == 2u,
                                     active == DEVICE_PROFILE_CUSTOM_REMAP),
                     "%c EDIT CUSTOM", g_mode_selection == 2u ? '>' : ' ');
            set_line(5, COLOR_GRAY, "JOY PRESS SELECT");
            set_line(6, COLOR_GRAY, "K1 BACK/CANCEL");
            set_line(7, COLOR_GRAY, "K3 HELP");
            break;
        }

        case SCREEN_CUSTOM:
            set_title("CUSTOM PENDING");
            ...
            set_line(6, COLOR_GRAY, "K2 ACTIONS");
            set_line(7, COLOR_GRAY, "K1 CANCEL  K3 HELP");
            break;

        case SCREEN_CAPTURE:
            set_title("SET TARGET %s", remap_target_name(g_capture_target));
            set_line(2, COLOR_WHITE, "PRESS MOUSE BUTTON");
            set_line(4, COLOR_WHITE, "MOVE/WHEEL IGNORED");
            set_line(6, COLOR_GRAY, "K1 CANCEL");
            set_line(7, COLOR_GRAY, "K3 HELP");
            break;

        case SCREEN_DRAG: {
            set_title("DRAG FIX %s",
                      g_drag_source == REMAP_SOURCE_FORWARD ? "FORWARD" : "BACK");
            const int applied = applied_drag_selection();
            set_line(1, option_color(g_drag_selection == 0u, applied == 0),
                     "%c AUTO", g_drag_selection == 0u ? '>' : ' ');
            set_line(2, option_color(g_drag_selection == 1u, applied == 1),
                     "%c FORCE IF SUPPORTED", g_drag_selection == 1u ? '>' : ' ');
            set_line(3, option_color(g_drag_selection == 2u, applied == 2),
                     "%c OFF", g_drag_selection == 2u ? '>' : ' ');
            set_line(4, COLOR_WHITE, "OFF = STANDARD HID");
            set_line(5, COLOR_WHITE, "NO HELD-STATE FIX");
            set_line(6, COLOR_GRAY, "JOY PRESS SELECT");
            set_line(7, COLOR_GRAY, "K1 BACK  K3 HELP");
            break;
        }

        case SCREEN_CUSTOM_ACTIONS:
            set_title("CUSTOM ACTIONS");
            set_line(1, option_color(g_action_selection == 0u, false),
                     "%c APPLY", g_action_selection == 0u ? '>' : ' ');
            set_line(2, option_color(g_action_selection == 1u, false),
                     "%c CLEAR MAPPINGS", g_action_selection == 1u ? '>' : ' ');
            set_line(4, COLOR_WHITE, "APPLY SAVES PENDING");
            set_line(5, COLOR_GRAY, "JOY PRESS SELECT");
            set_line(6, COLOR_GRAY, "K1 BACK/CANCEL");
            set_line(7, COLOR_GRAY, "K3 HELP");
            break;
        ...
        case SCREEN_CLEAR_CONFIRM:
            set_title("CLEAR MAPPINGS");
            set_line(2, COLOR_WHITE, "CLEAR SAVED CUSTOM?");
            set_line(3, COLOR_WHITE, "ALL BUTTONS PASS");
            set_line(4, COLOR_WHITE, "DRAG FIX -> AUTO");
            set_line(5, COLOR_GRAY, "JOY PRESS CONFIRM");
            set_line(6, COLOR_GRAY, "K1 CANCEL");
            set_line(7, COLOR_GRAY, "K3 HELP");
            break;
        ...
        case SCREEN_WAIT_APPLY:
            set_title("APPLYING");
            set_line(2, COLOR_WHITE, "NEUTRALIZING USB");
            set_line(4, COLOR_WHITE, "SAVING TO DEVICE");
            break;
        ...
        case SCREEN_ERROR:
            set_title("ERROR");
            set_line(2, COLOR_WHITE, "%s", g_error);
            set_line(6, COLOR_GRAY, "K1 BACK");
            set_line(7, COLOR_GRAY, "JOY PRESS STATUS");
            break;
    }
}
...
static void render_task(void)
{
    ...
    switch (g_render_phase) {
        ...
        case RENDER_TEXT: {
            ...
            if (c != ' ') {
                const uint16_t x = (uint16_t)(TEXT_X +
                    (uint16_t)g_text_char * CHAR_ADVANCE);
                const uint16_t y = (uint16_t)(TEXT_Y +
                    (uint16_t)g_text_line * LINE_ADVANCE);
                if (x + DRAW_W <= LCD_WIDTH && y + DRAW_H <= LCD_HEIGHT) {
                    draw_glyph(x, y, c, g_line_colors[g_text_line]);
                }
            }
            ++g_text_char;
            break;
...
static void drain_source_events(void)
{
    ...
    while (remap_engine_poll_source_down(&source)) {
        (void)source;
    }
}
...
static void select_mode(void)
{
    ...
    if (g_mode_selection == 0u) {
        config.mode = DEVICE_PROFILE_PASSTHROUGH;
        request_apply(&config, APPLY_KIND_PASSTHROUGH, SCREEN_MODE_MENU);
    } else if (g_mode_selection == 1u) {
        ...
        request_apply(&config, APPLY_KIND_DEFAULT, SCREEN_MODE_MENU);
    } else {
        g_pending = config;
...
static void handle_pressed(pico_hat_input_t input)
{
    ...
    if (input == PICO_HAT_INPUT_KEY3 &&
        g_screen != SCREEN_HELP &&
        g_screen != SCREEN_WAIT_APPLY &&
        g_screen != SCREEN_SUCCESS &&
    ...
    switch (g_screen) {
        case SCREEN_HOME:
            if (input == PICO_HAT_INPUT_JOY_PRESS) {
                ...
                g_mode_selection = g_have_profile ? (uint8_t)g_profile.active.mode : 0u;
                if (g_mode_selection > 2u) g_mode_selection = 0u;
        ...
        case SCREEN_MODE_MENU:
            if (input == PICO_HAT_INPUT_JOY_UP) {
                g_mode_selection = g_mode_selection == 0u
                    ? 2u : (uint8_t)(g_mode_selection - 1u);
                g_dirty = true;
        ...
        case SCREEN_DRAG:
            if (input == PICO_HAT_INPUT_JOY_UP) {
                g_drag_selection = g_drag_selection == 0u
                    ? 2u : (uint8_t)(g_drag_selection - 1u);
                g_dirty = true;
            } else if (input == PICO_HAT_INPUT_JOY_DOWN) {
                g_drag_selection = (uint8_t)((g_drag_selection + 1u) % 3u);
                g_dirty = true;
            } else if (input == PICO_HAT_INPUT_JOY_PRESS) {
                const device_drag_fix_policy_t policy =
                    selection_to_policy(g_drag_selection);
        ...
        case SCREEN_CUSTOM_ACTIONS:
            if (input == PICO_HAT_INPUT_JOY_UP || input == PICO_HAT_INPUT_JOY_DOWN) {
                g_action_selection = (uint8_t)(1u - g_action_selection);
                g_dirty = true;
            } else if (input == PICO_HAT_INPUT_JOY_PRESS) {
                if (g_action_selection == 0u) {
                    g_pending.mode = DEVICE_PROFILE_CUSTOM_REMAP;
                    request_apply(&g_pending, APPLY_KIND_CUSTOM, SCREEN_CUSTOM_ACTIONS);
                } else {
        ...
        case SCREEN_CLEAR_CONFIRM:
            if (input == PICO_HAT_INPUT_JOY_PRESS) {
                ...
                request_apply(&g_pending, APPLY_KIND_CLEAR, SCREEN_CUSTOM_ACTIONS);
            } else if (input == PICO_HAT_INPUT_KEY1) {
                set_screen(SCREEN_CUSTOM_ACTIONS);
            }
            break;
...
static void refresh_snapshots(void)
{
    ...
    if (remap_profile_get_snapshot(&profile) && profile.revision != g_profile_revision) {
        ...
        g_profile = profile;
        g_have_profile = true;
        if (g_screen == SCREEN_STATUS) g_dirty = true;

        if (g_screen == SCREEN_WAIT_APPLY && g_wait_apply_id != 0u &&
            profile.last_apply_id == g_wait_apply_id) {
            g_wait_apply_id = 0u;
            if (profile.last_apply_ok) set_screen(SCREEN_SUCCESS);
            else show_error("APPLY FAILED", g_feedback_return);
        }
    }
...
void pico06_ui_init(void)
{
    ...
    g_profile_revision = 0u;
    g_hidpp_revision = 0u;
    g_screen = SCREEN_HOME;
    g_help_return = SCREEN_HOME;
    g_feedback_return = SCREEN_HOME;
    g_render_phase = RENDER_WAIT_LCD;
    g_dirty = true;
    g_was_locked = false;
    g_wait_apply_id = 0u;
    g_apply_kind = APPLY_KIND_NONE;
