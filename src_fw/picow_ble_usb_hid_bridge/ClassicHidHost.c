...
static bt_host_device_kind_t classic_hid_kind_from_name(const char *name)
{
    ...
    if (keyboard && mouse) return BT_HOST_DEVICE_KIND_KEYBOARD_MOUSE;
    if (keyboard) return BT_HOST_DEVICE_KIND_KEYBOARD;
    if (mouse) return BT_HOST_DEVICE_KIND_MOUSE;
    return BT_HOST_DEVICE_KIND_UNKNOWN;
}
...
{
    if (device == NULL || name == NULL) return;
...
static int classic_hid_device_index_for_address(const bd_addr_t address)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        if (bd_addr_cmp(address, s_devices[i].address) == 0) return (int)i;
    }
    return -1;
}
...
{
    if (command == CLASSIC_COMMAND_NONE || s_pending_command != CLASSIC_COMMAND_NONE) return false;
...
static void classic_hid_set_target_from_remembered(void)
{
    ...
    (void)snprintf(s_target_info.address, sizeof(s_target_info.address), "%s", bd_addr_to_str(s_target_addr));
    if (s_remembered_device.name[0] != '\0') {
        (void)snprintf(s_target_info.name, sizeof(s_target_info.name), "%s", s_remembered_device.name);
        (void)snprintf(s_device_name, sizeof(s_device_name), "%s", s_remembered_device.name);
    } else {
        (void)snprintf(s_device_name, sizeof(s_device_name), "%s", s_target_info.address);
    }
...
{
    return s_has_remembered_device && bd_addr_cmp(s_target_addr, s_remembered_device.address) == 0;
}
...
static void classic_hid_forget_remembered_on_core1(void)
{
    ...
    classic_hid_publish_remembered(NULL);
    if (s_state == BT_HOST_STATE_RECONNECTING) classic_hid_start_inquiry_on_core1();
}
...
static void classic_hid_finish_discovery(void)
{
    ...
    printf("[G04][ClassicHID] discovery complete: %u device(s), selected=%d\n",
           (unsigned int)s_device_count, (int)s_selected_index);
}
...
static void classic_hid_start_inquiry_on_core1(void)
{
    ...
    classic_hid_cancel_reconnect_timer();
    gap_inquiry_stop();
    if (previous_cid != 0) hid_host_disconnect(previous_cid);
    ...
    if (had_descriptor) g_usb_reinit_request = true;
...
static void classic_hid_request_next_remote_name(void)
{
    ...
    const size_t count = s_device_count;
    for (size_t i = 0; i < count; ++i) {
        if (s_devices[i].name_state != DEVICE_NAME_UNKNOWN) continue;
        s_devices[i].name_state = DEVICE_NAME_REQUESTED;
        const int status = gap_remote_name_request(s_devices[i].address,
...
{
    if (s_state != BT_HOST_STATE_DISCOVERING) return;
    ...
    if (s_device_count >= BT_HOST_MAX_DEVICES) return;
    ...
    (void)snprintf(device->info.address, sizeof(device->info.address), "%s", bd_addr_to_str(address));
...
{
    if (s_state != BT_HOST_STATE_RESOLVING_NAMES) return;
    bd_addr_t address;
    ...
    if (packet[2] == ERROR_CODE_SUCCESS) {
        classic_hid_copy_name(device, (const char *)&packet[9]);
    } else {
        classic_hid_refresh_device_classification(device);
    }
    classic_hid_request_next_remote_name();
...
static void classic_hid_connect_selected_on_core1(void)
{
    if (s_state != BT_HOST_STATE_DEVICE_SELECTION) return;
    const int selected = s_selected_index;
    if (selected < 0 || (size_t)selected >= s_device_count || !s_devices[selected].info.hid_candidate) {
        s_state = BT_HOST_STATE_ERROR;
        return;
    }
    ...
    s_target_info_valid = true;
    (void)snprintf(s_device_name, sizeof(s_device_name), "%s",
                   device->info.name[0] != '\0' ? device->info.name : device->info.address);

    classic_hid_clear_connection_state();
    s_state = BT_HOST_STATE_CONNECTING;
    gap_inquiry_stop();
    printf("[G04][ClassicHID] connecting to selected HID candidate %s (%s)\n",
           device->info.address, s_device_name);
...
{
    if (bd_addr_cmp(address, s_target_addr) != 0) return;
    ...
    if (method == BT_HOST_PAIRING_LEGACY_PIN) {
        (void)snprintf(s_pairing_info.legacy_pin, sizeof(s_pairing_info.legacy_pin), "%s", s_legacy_pin);
    }
...
{
    if (!s_descriptor_available || report_len < 2 || report[0] != 0xA1) return;
    static ST_HID_RPT usb_report;
    usb_report.report_id = report[1];
    usb_report.report_len = report_len - 1;
    if (usb_report.report_len > CMN_HID_RPT_DATA_SIZE) usb_report.report_len = CMN_HID_RPT_DATA_SIZE;
...
{
    classic_hid_enqueue_usb_report(hid_subevent_report_get_report(packet),
...
static void classic_hid_execute_pending_command(void)
{
    ...
    if (command == CLASSIC_COMMAND_NONE) return;
    ...
    switch (command) {
        ...
        case CLASSIC_COMMAND_PAIRING_ACCEPT:
            if (!s_pairing_addr_valid || !s_pairing_info.action_required) break;
        ...
        case CLASSIC_COMMAND_SUBMIT_PASSKEY:
            if (s_pairing_addr_valid &&
                ...
                s_pairing_info.action_required && arg <= 999999u) {
...
static void classic_hid_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    UNUSED(channel);
    UNUSED(size);
    ...
    switch (hci_event_packet_get_type(packet)) {
        case BTSTACK_EVENT_STATE:
            if (btstack_event_state_get_state(packet) == HCI_STATE_WORKING) {
                ...
                if (s_has_remembered_device) classic_hid_begin_auto_reconnect_on_core1();
                else classic_hid_start_inquiry_on_core1();
            }
            break;
        ...
        case GAP_EVENT_INQUIRY_COMPLETE:
            if (s_state == BT_HOST_STATE_DISCOVERING) classic_hid_request_next_remote_name();
            break;
        ...
        case HCI_EVENT_PIN_CODE_REQUEST:
            ...
            classic_hid_prepare_pairing(event_addr, BT_HOST_PAIRING_LEGACY_PIN, 0, true);
            break;
        case HCI_EVENT_USER_CONFIRMATION_REQUEST:
            hci_event_user_confirmation_request_get_bd_addr(packet, event_addr);
            classic_hid_prepare_pairing(event_addr,
        ...
        case HCI_EVENT_USER_PASSKEY_REQUEST:
            ...
            classic_hid_prepare_pairing(event_addr, BT_HOST_PAIRING_PASSKEY_INPUT, 0, true);
            break;
        case HCI_EVENT_SIMPLE_PAIRING_COMPLETE:
            if (hci_event_simple_pairing_complete_get_status(packet) != ERROR_CODE_SUCCESS) {
                if (s_auto_reconnect_active) classic_hid_schedule_reconnect_retry_on_core1();
                else s_state = BT_HOST_STATE_ERROR;
            }
            break;
        case HCI_EVENT_HID_META:
            switch (hci_event_hid_meta_get_subevent_code(packet)) {
                case HID_SUBEVENT_INCOMING_CONNECTION: {
                    ...
                    const bool target_matches = bd_addr_cmp(incoming_addr, s_target_addr) == 0;
                    if ((s_state == BT_HOST_STATE_CONNECTING ||
                         s_state == BT_HOST_STATE_RECONNECTING ||
                         s_state == BT_HOST_STATE_PAIRING) && target_matches) {
                ...
                case HID_SUBEVENT_CONNECTION_CLOSED: {
                    ...
                    classic_hid_clear_connection_state();
                    if (had_descriptor) g_usb_reinit_request = true;
                    if (manual_scan_active) break;
                    if (s_auto_reconnect_active && s_reconnect_timer_scheduled) break;
...
{
    return s_state == BT_HOST_STATE_READY && s_descriptor_available && s_hid_host_cid != 0;
}

const uint8_t *CLASSIC_HID_GetReportDescriptor(void)
{
    if (!CLASSIC_HID_IsReady()) return NULL;
...
{
    if (!CLASSIC_HID_IsReady()) return 0;
    return hid_descriptor_storage_get_descriptor_len((uint16_t)s_hid_host_cid);
}

bt_host_state_t CLASSIC_HID_GetState(void) { return s_state; }
...
const char *CLASSIC_HID_GetDeviceName(void) { return s_device_name; }
size_t CLASSIC_HID_GetDiscoveredDeviceCount(void) { return s_device_count; }

bool CLASSIC_HID_GetDiscoveredDevice(size_t index, bt_host_device_t *out_device)
{
    if (out_device == NULL || index >= s_device_count) return false;
    *out_device = s_devices[index].info;
    return true;
}

int CLASSIC_HID_GetSelectedDeviceIndex(void) { return s_selected_index; }

bool CLASSIC_HID_SelectDevice(size_t index)
{
    if (s_state != BT_HOST_STATE_DEVICE_SELECTION || index >= s_device_count || !s_devices[index].info.hid_candidate) return false;
...
{
    if (s_state != BT_HOST_STATE_DEVICE_SELECTION || s_device_count == 0) return false;
    const size_t count = s_device_count;
...
{
    if (s_state != BT_HOST_STATE_DEVICE_SELECTION || s_selected_index < 0 ||
        (size_t)s_selected_index >= s_device_count || !s_devices[s_selected_index].info.hid_candidate) return false;
...
bool CLASSIC_HID_StartDiscovery(void)
{
    if (s_state != BT_HOST_STATE_DEVICE_SELECTION && s_state != BT_HOST_STATE_ERROR &&
        s_state != BT_HOST_STATE_RECONNECTING && s_state != BT_HOST_STATE_CONNECTED &&
        s_state != BT_HOST_STATE_READY) return false;
...
{
    if (out_device == NULL || !s_has_remembered_device) return false;
    ...
    (void)snprintf(out_device->address, sizeof(out_device->address), "%s", bd_addr_to_str(address));
...
{
    if (!s_has_remembered_device && !s_auto_reconnect_active) return false;
    return classic_hid_queue_command(CLASSIC_COMMAND_FORGET_DEVICE, 0);
}

bt_host_pairing_info_t CLASSIC_HID_GetPairingInfo(void) { return s_pairing_info; }

bool CLASSIC_HID_ConfirmPairing(bool accept)
{
    if (s_state != BT_HOST_STATE_PAIRING || !s_pairing_info.action_required) return false;
    if (!accept) {
        if (s_pairing_info.method != BT_HOST_PAIRING_LEGACY_PIN &&
            s_pairing_info.method != BT_HOST_PAIRING_NUMERIC_CONFIRMATION &&
            s_pairing_info.method != BT_HOST_PAIRING_PASSKEY_INPUT) return false;
        return classic_hid_queue_command(CLASSIC_COMMAND_PAIRING_REJECT, 0);
    }
    if (s_pairing_info.method != BT_HOST_PAIRING_LEGACY_PIN &&
        s_pairing_info.method != BT_HOST_PAIRING_NUMERIC_CONFIRMATION) return false;
    return classic_hid_queue_command(CLASSIC_COMMAND_PAIRING_ACCEPT, 0);
}

bool CLASSIC_HID_SubmitPasskey(uint32_t passkey)
{
    if (s_state != BT_HOST_STATE_PAIRING || s_pairing_info.method != BT_HOST_PAIRING_PASSKEY_INPUT ||
        !s_pairing_info.action_required || passkey > 999999u) return false;
